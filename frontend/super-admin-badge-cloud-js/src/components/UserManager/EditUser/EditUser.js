...
const EditUser = () => {
    ...
    const editCategoryByTenantId = async () => {
        try {
            const response = await UserApi.UserCategoryByTenantId();
    ...
    const getById = async (id) => {
        try {
            ...
            let getCategory = await UserCategoryByTenantId();
            getCategory = getCategory?.filter(item => item.category === response?.category)
            setUserGategory(getCategory)
            setRecords(response);
            setImage(imgBaseUrl + response?.photo);
        } catch (error) { }
    };
    ...
    return (
        <Formik
            initialValues={{
                ...
                photo: records?.photo || null,
                categoryName: records?.category || ''
            }}
            ...
            onSubmit={async (
                ...
                try {
                    console.log("values mani", values);
                    ...
                    setSubmitting(true);
                    const res = await UserApi.edit(formData, location.state.id);
                    await dispatch(
                                                ...
                                                <div className="row mb-4">
                                                    <div className="col-lg-12">
                                                        ...
                                                        <select className="form-select" id="category" name="categoryName" required
                                                            onClick={() => editCategoryByTenantId()}
                                                            onChange={(e) => {
                                                        ...
                                                        >
                                                            {userCatergory.length > 0 && userCatergory.map((category) => {
                                                                return (
                                                                    <option key={category.id} value={category.value} className="administrator">{category?.category}</option>
                                                                )
                                                            })
                                                            }
