...
const BadgeList = () => {
  ...
  const getAllList = async () => {
    try {
      const response = await BadgeApi.getAll(tenantId, page, limit);
  ...
  const editById = async (data) => {
    ...
    try {
      const res = await BadgeApi.edit(body);
      await dispatch(
  ...
  const getAndDeleteById = async () => {
    try {
      await BadgeApi.deleteById(deleteRecord);
            ...
            <div className='col-12'>
              <div className='page-title-box'>
                <div className='page-title-right'>
                  <Link to='/badge/add-badge' className='btn btn-primary'>
                    Add Badge
                  </Link>
                </div>
                <h4 className='page-title'>Badges Collection List</h4>
              </div>
            </div>
          </div>
          <div className='row'>
                          ...
                          <thead>
                            <tr>
                              ...
                              <th
                                ...
                                colSpan={1}
                                style={{ width: '416.8px' }}
                                aria-label='badge Name: activate to sort column ascending'
                              >
                                Collection Image
                              </th>
                              <th
                            ...
                            {records?.map((record, index) => {
                              return (
                                <tr key={record?.id}>
                                  ...
                                  <td>
                                    <img src={record?.fileName ? imgBaseUrl + record?.fileName : '../assets/img/user.png'} className={classes.img} onLoad={() => setLoaded(true)}

                                      style={{
                                        background: `url(${loaded ? "../assets/img/user.png" : ""})`
                                      }}
                                    />
                                  </td>
                                  <td>
                                    <div className='dropdown'>
                                      ...
                                      <div className='dropdown-menu dropdown-menu-animated'>
                                        ...
                                        <a href="#preview" data-bs-toggle="modal" role="button" class="dropdown-item" onClick={()=>{
                                          console.log("recodfsfs", record)
                                          setPreview(record)}}>Preview</a>
                                        <Link
                                          to='/badge/edit-badge'
                                          class='dropdown-item'
                                          state={{ id: record?.id }}
